...
class VmPlacementMaxFit:
    def get_fit_map(self, host_list: List[Host], vm_list: List[Vm]) -> Optional[Dict]:
        def host_suitability_comparator(suitability_a: HostSuitability, suitability_b: HostSuitability) -> bool:
            ...
            if suitability_a.get_is_suitable() and not suitability_b.get_is_suitable():
                return True
            # if host a is not suitable for vm while b is
            elif not suitability_a.get_is_suitable() and suitability_b.get_is_suitable():
                return False
            # if both of host a and host b are not suitable for vm, their order doesn't matter
            elif not suitability_a.get_is_suitable() and not suitability_b.get_is_suitable():
                return False
            # if both of host a and host b are suitable for vm, the one with more available CPU cores goes first
            else:
                if suitability_a.get_host().get_num_available_pes() > suitability_b.get_host().get_num_available_pes():
                    return True
                elif suitability_a.get_host().get_num_available_pes() < suitability_b.get_host().get_num_available_pes():
                    return False
        ...
        for vm in vm_list:
            for host_suitability in host_suitability_heap:
                host_suitability.is_suitable_for(vm)
            host_suitability_heap.reheapify()
            suitability_head = host_suitability_heap.pop()
            if suitability_head.get_is_suitable() == False:
